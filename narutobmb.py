...
👤 <b>User:</b> {display_name(user_id, username, None)}
...
""")
        ...
        await context.bot.send_message(owner_id, message, parse_mode="HTML")
...
def log_bombing(user_id, username, phone, duration, is_vip, status="STARTED"):
    ...
    try:
        with open("bombing_log.txt", "a") as f:
            f.write(log_entry)
    except Exception:
        pass
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Stop an active bombing job"""
    user_id = update.effective_user.id
    args = context.args

    # If no args, show active jobs
    if not args:
